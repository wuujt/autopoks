...
export class Damage {
  static typeChart: Record<string, Partial<Record<string, number>>> = {
  ...
  static getEffectiveness(attackType: string, defenseType: string): number {
    return this.typeChart[attackType]?.[defenseType] ?? 1;
  }
  ...
  static calculateDamage(
    ...
    const stab =
      move?.type === attacker.type1 || move?.type === attacker.type2 ? 1.5 : 1;
    const level = 50;
    if (move) {
